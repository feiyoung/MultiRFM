...
nu <- nu.vec[j]
p <- 50
nvec <- c(100,150); d <- 3; q <- 3;qs <- c(2,2); S <- length(nvec)
sigma2_eps <- 1
N <- 100
method_run <- 'All'
...
save(metricList, file=paste0("scenario4",dir.name,"_",method_run,"nsum", sum(nvec),"nu",paste0(nu, collapse = "_"),"q", q ,"p",p,"N",N, '_metricList.rds'))

}

#metricList$mu_er
#metricList$A_tr
print("nvec=c(100,150)")
print("p=50")
